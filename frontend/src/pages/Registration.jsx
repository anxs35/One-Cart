    ...
    <div className='w-full min-h-screen bg-slate-100 text-slate-900 flex flex-col items-center justify-center p-4'>
        ...
        <div className='w-full max-w-md bg-white p-8 md:p-10 rounded-xl shadow-lg'>
            <div className='text-center mb-8'>
                ...
                <p className='text-slate-600 mt-2'>Join OneCart today.</p>
            </div>
            
            <form onSubmit={handleSignup} className='w-full flex flex-col gap-5'>
                <button type="button" onClick={googleSignup} className='w-full h-12 border border-slate-300 rounded-lg flex items-center justify-center gap-3 text-slate-700 font-medium hover:bg-slate-50 transition-colors'>
                ...
                <div className='w-full flex flex-col gap-4 relative'>
                    <input type="text" className='w-full h-12 border border-slate-300 rounded-lg bg-transparent px-4 font-medium placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500' placeholder='Full Name' required onChange={(e)=>setName(e.target.value)} value={name}/>
                    ...
                    <p className='text-center text-sm text-slate-600 mt-4'>
                        Already have an account? <span className='text-blue-600 font-semibold cursor-pointer hover:underline' onClick={()=>navigate("/login")}>Login</span>
                    </p>
                </div>
            </form>
        </div>
